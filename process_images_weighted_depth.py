...
def initialize_depth_model(encoder, max_depth, load_from):
...
def predicted_depth2pcd(model, rgb, cam_int, cam_ext, mask):
    ...
    # print dimensions of resized_pred
    print("resized_pred.size", resized_pred.size)
...
def main():
    ...
    depth_model = initialize_depth_model(
        "vitl",
        20,
        "/local/home/andrye/dev/Depth-Anything-V2/metric_depth/checkpoints/depth_anything_v2_metric_hypersim_vitl.pth",
    )
    ...
    # clean model and all others from memory
    del model
    del model_cfg
    del detector
    del cpm
    ...
    predicted_ego_depth_pcd = predicted_depth2pcd(
        depth_model, ego_rgb, ego_cam_int, ego_cam_ext, ego_mask
    )
    predicted_exo_depth_pcd = predicted_depth2pcd(
        depth_model, exo_rgb, exo_cam_int, exo_cam_ext, exo_mask
    )
